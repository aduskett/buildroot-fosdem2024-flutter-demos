...
ENV_FILES ?= "fosdem.json"
EXIT_AFTER_BUILD ?= false
NO_BUILD ?= false
VERBOSE ?= false
TARGET ?= x64_virt_flutter_demo
...
help:
	...
	@printf "\tENV_FILES: Specify a space-deliminated list of environment files in the docker directory to apply. Default: fosdem.json\n"
	...
	@printf "\tTARGET: Specify the target of which to run menuconfig. Default: x64_virt_flutter_demo\n"
	@printf "\n"
	@printf "Targets:\n"
	...
	@printf "\tmenuconfig: run menuconfig on a given target. Default: x64_virt_flutter_demo\n"
	...
	@printf "\n"
	@printf "Other:\n"
	@printf "x64-run: Run the x64 virtual image"
	@printf "\n\n"
...
menuconfig:
	...
	@docker exec -it buildroot-fosdem-build /bin/bash -c 'cd ~/buildroot/external/output/${TARGET} && make menuconfig'
...
.PHONY: shell
shell:
	@/bin/bash -c 'if [ -z $(docker container ps -aqf "name=buildroot-fosdem-build") ]; then make APPLY_CONFIGS=${APPLY_CONFIGS} NO_BUILD=true EXIT_AFTER_BUILD=false upd; fi;'
	@/bin/bash -c 'if [[ ${APPLY_CONFIGS} == "true" ]]; then sleep 5; fi'
	@docker exec -it buildroot-fosdem-build /bin/bash
...
.PHONY: x64-run
x64-run:
	@/bin/bash fosdem/board/x64-virt/qemu-run.sh
